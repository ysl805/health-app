    ...
    async def __call__(self, scope, receive, send):
        ...
        async def send_wrapper(message):
            if message["type"] == "http.response.start":
                ...
                # Cache-Control (代替 Expires)
                path = scope.get("path", "")
                if path.endswith(".html"):
                    headers[b"cache-control"] = b"no-cache, no-store, must-revalidate"
                elif path.startswith("/libs/") or path.startswith("/css/") or path.startswith("/js/"):
                    headers[b"cache-control"] = b"public, max-age=31536000, immutable"
                else:
                    headers[b"cache-control"] = b"no-cache"
                # 移除 X-Frame-Options (用 CSP frame-ancestors 代替)
                # 移除 X-XSS-Protection (已废弃)
                if b"x-frame-options" in headers:
                    del headers[b"x-frame-options"]
                if b"x-xss-protection" in headers:
                    del headers[b"x-xss-protection"]
                # CSP (包含 frame-ancestors 'self')
                if b"content-security-policy" not in headers:
                    csp = (
                        "default-src 'self'; "
                        "script-src 'self' 'unsafe-inline' https://fonts.loli.net; "
...
from models import User, KnowledgeBase, KnowledgeDocument, KnowledgeBaseBinding, Consultation, TongueImage, LocalCase, Inventory
...
FRONTEND_DIR = pathlib.Path(__file__).parent.parent / "frontend" / "www"
...
class UserCreate(BaseModel):
    ...
    org_name: str = ""  # 单位名称
    region_id: Optional[int] = None  # 绑定区域
...
class UserUpdate(BaseModel):
    ...
    valid_until: Optional[str] = None
    manager_id: Optional[int] = None  # 上级负责人
    phone: Optional[str] = None  # 手机号
    org_name: Optional[str] = None  # 单位名称
    region_id: Optional[int] = None  # 绑定区域

class InventoryCreate(BaseModel):
    user_id: int  # 属于哪个客户
    knowledge_base_id: Optional[int] = None  # 关联知识库（产品来源）
...
async def login(request: LoginRequest, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "user": {
            ...
            "region": user.region,
            "knowledge_base_ids": [kb.knowledge_base_id for kb in user.knowledge_base_bindings],
        },
    }
...
async def list_users(
    ...
    if current_user.role != "super_admin":
        ...
        # Further filter by region: only users in subordinate regions
        if current_user.region_id:
            subordinate_region_ids = get_subordinate_region_ids(current_user.region_id, db)
            query = query.filter(User.region_id.in_(subordinate_region_ids))
        else:
            # If current user has no region, they can't see any subordinate users
    ...
    return [{
        ...
        "region": u.region, "is_active": u.is_active, "org_name": u.org_name or "", "region_id": u.region_id,
...
async def update_user(
    ...
    if user_data.org_name is not None:
        target.org_name = user_data.org_name
    if user_data.region_id is not None:
        target.region_id = user_data.region_id
    if user_data.phone is not None:
        target.phone = user_data.phone
    if user_data.manager_id is not None:
        target.manager_id = user_data.manager_id
    if hasattr(user_data, "knowledge_base_ids") and user_data.knowledge_base_ids is not None:
        db.query(KnowledgeBaseBinding).filter(KnowledgeBaseBinding.user_id == target.id).delete()
        for kb_id in user_data.knowledge_base_ids:
            binding = KnowledgeBaseBinding(user_id=target.id, knowledge_base_id=kb_id)
            db.add(binding)
...
async def list_consultations(
    ...
    # Role-based data access (using region tree)
    if current_user.role == "super_admin":
        pass  # Can see all consultations
    else:
        # Get subordinate user IDs (including self)
        subordinate_ids = _get_subordinate_ids(current_user, db)
        query = query.filter(Consultation.user_id.in_(subordinate_ids))
...
@app.get("/api/consultations/{consultation_id}/tongue-image")
async def get_consultation_tongue_image(
    consultation_id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    tongue_img = db.query(TongueImage).filter(TongueImage.consultation_id == consultation_id).first()
    if not tongue_img:
        return {"found": False}
    return {
        "found": True,
        "image_path": tongue_img.image_path if tongue_img.image_path else "",
    }
...
async def create_local_case(
    ...
    q, ans, tongue, syndrome, symptoms_data, tongue_img_path = "", "", "", "", "", ""
    ...
    if consultation_id:
        ...
        if record:
            ...
            # Find tongue image for this consultation

            tongue_img = db.query(TongueImage).filter(TongueImage.consultation_id == consultation_id).first()

            if tongue_img and tongue_img.image_path:

                tongue_img_path = tongue_img.image_path

    # Handle tongue image base64 passed from frontend

    tongue_image_b64 = data.get("tongue_image_base64")

    if tongue_image_b64:

        img_data = base64.b64decode(tongue_image_b64)

        ts = datetime.utcnow().strftime('%Y%m%d_%H%M%S')

        img_path = os.path.join(UPLOAD_DIR, f"local_tongue_{ts}_{current_user.id}.jpg")

        with open(img_path, "wb") as f:

            f.write(img_data)

        tongue_img_path = img_path
    ...
    case = LocalCase(
        ...
        tongue_image_path=tongue_img_path or data.get("tongue_image_path", ""),
...
async def list_local_cases(
    ...
    return [{
        ...
        "tongue_image_path": "/uploads/" + os.path.basename(c.tongue_image_path) if c.tongue_image_path else "",
...
async def get_local_case(
    ...
    return {
        ...
        "tongue_image_path": "/uploads/" + os.path.basename(case.tongue_image_path) if case.tongue_image_path else "",
...
def _get_subordinate_ids(user: User, db: Session) -> List[int]:
    ...
    # For employee (普通用户), only return self
    if user.role == 'employee':
        return [user.id]
    
    # For admin roles, get users in subordinate regions
    if user.region_id:
        subordinate_region_ids = get_subordinate_region_ids(user.region_id, db)
        # Get all user IDs in these regions
        query = db.query(User.id).filter(
            User.region_id.in_(subordinate_region_ids),
            User.is_active == True
        )
        return [r[0] for r in query.all()]
    else:
        # No region_id, only return self
        return [user.id]
...
def _format_consultation(record: Consultation, db: Session) -> dict:
    ...
    if user and user.region_id:
        region_path = get_region_path(user.region_id, db)
...
# Mount libs directory (for vue, element-plus etc)
static_dir = FRONTEND_DIR / "libs"
if static_dir.exists():
    app.mount("/libs", StaticFiles(directory=str(static_dir)), name="libs")
...
async def list_managers(
    ...
    """获取可选的业务经理列表"""
    if current_user.role == "super_admin":
        managers = db.query(User).filter(User.role == "business_manager", User.is_active == True).all()
    elif current_user.role in ("admin", "province_manager", "region_manager"):
        managers = db.query(User).filter(User.role == "business_manager", User.is_active == True).all()
    elif current_user.role == "business_manager":
        managers = [current_user]
    else:
        managers = []
    return [{"id": m.id, "username": m.username, "real_name": m.real_name or m.username, "role": m.role} for m in managers]


@app.get("/api/subordinates")
async def list_subordinates(
    current_user: User = Depends(require_role("business_manager", "region_manager", "province_manager", "admin", "super_admin")),
    db: Session = Depends(get_db),
):
    """获取下属用户列表"""
    if current_user.role == "super_admin":
        ...
        users = db.query(User).filter(User.manager_id == current_user.id, User.is_active == True).all()
    else:
        allowed_roles = SUBORDINATE_ROLES.get(current_user.role, [])
        users = db.query(User).filter(User.role.in_(allowed_roles), User.is_active == True).all()
    return [{"id": u.id, "username": u.username, "real_name": u.real_name or u.username, "role": u.role, "phone": u.phone or "", "manager_id": u.manager_id} for u in users]


@app.post("/api/bind-manager")
async def bind_manager(
    ...
    if manager_id:
        ...
        if not manager or manager.role != "business_manager":
            raise HTTPException(status_code=400, detail="只能绑定业务经理作为上级负责人")
        target.manager_id = manager_id
    else:
        target.manager_id = None
    db.commit()
    return {"message": "绑定成功", "manager_id": target.manager_id}


# ── Inventory CRUD API ──────────────────────────
# 权限规则：
#   普通用户(user): 可读写自己的库存
#   业务经理(business_manager): 可读写下属客户的库存
#   管理层(admin/province_manager/region_manager/super_admin): 只读查看，不可修改

READ_ROLES = ["super_admin", "admin", "province_manager", "region_manager", "business_manager", "user"]
WRITE_ROLES = ["business_manager", "user"]  # 普通用户和业务经理可写
OWNER_WRITE_ROLES = ["business_manager", "user"]  # 可修改库存的角色
...
async def list_inventory(
    ...
    if current_user.role == "user":
        ...
        if user_id:
            if user_id == current_user.id:
                ...
                sub = db.query(User).filter(User.id == user_id, User.manager_id == current_user.id).first()
                if not sub:
                    raise HTTPException(status_code=403, detail="该用户不是您的下属客户")
            ...
            items = db.query(Inventory).filter(Inventory.user_id.in_(subordinate_ids)).all()
    else:
        # 管理层：只读查看，可按用户筛选或看全部
        if user_id:
            items = db.query(Inventory).filter(Inventory.user_id == user_id).all()
    ...
    # 批量获取用户名和知识库名避免 N+1
    all_user_ids = set(i.user_id for i in items)
    all_kb_ids = set(i.knowledge_base_id for i in items if i.knowledge_base_id)
    user_map = {u.id: u.real_name or u.username for u in db.query(User).filter(User.id.in_(all_user_ids)).all()}
    kb_map = {k.id: k.name for k in db.query(KnowledgeBase).filter(KnowledgeBase.id.in_(all_kb_ids)).all()} if all_kb_ids else {}

    return [{
        "id": i.id, "user_id": i.user_id, "knowledge_base_id": i.knowledge_base_id,
        "kb_name": kb_map.get(i.knowledge_base_id, "") if i.knowledge_base_id else "",
        "product_name": i.product_name, "specification": i.specification,
        "quantity": i.quantity, "unit": i.unit, "price": i.price, "cost_price": i.cost_price,
        "notes": i.notes, "created_by": i.created_by,
        "created_at": i.created_at.isoformat() + "+08:00" if i.created_at else None,
        "updated_at": i.updated_at.isoformat() + "+08:00" if i.updated_at else None,
...
async def create_inventory(
    ...
    """添加库存 - 普通用户给自己添加，业务经理给下属添加"""
    if current_user.role == "user":
        # 普通用户只能给自己添加库存
        if inv_data.user_id != current_user.id:
            raise HTTPException(status_code=403, detail="普通用户只能为自己添加库存")
    elif current_user.role == "business_manager":
        # 业务经理给自己或下属添加库存
        if inv_data.user_id != current_user.id:
            sub = db.query(User).filter(User.id == inv_data.user_id, User.manager_id == current_user.id).first()
            if not sub:
                raise HTTPException(status_code=403, detail="该用户不是您的下属客户，无法为其添加库存")

    item = Inventory(
        user_id=inv_data.user_id, knowledge_base_id=inv_data.knowledge_base_id,
        product_name=inv_data.product_name, specification=inv_data.specification,
        quantity=inv_data.quantity, unit=inv_data.unit, price=inv_data.price,
        cost_price=inv_data.cost_price, notes=inv_data.notes,
        created_by=current_user.id,
    )
    db.add(item)
    db.commit()
    db.refresh(item)
    return {"id": item.id, "message": "库存添加成功"}


@app.put("/api/inventory/{item_id}")
async def update_inventory(
    item_id: int, inv_data: InventoryUpdate,
    current_user: User = Depends(require_role(*WRITE_ROLES)),
    db: Session = Depends(get_db),
):
    """修改库存 - 普通用户改自己的，业务经理改下属的"""
    ...
    if current_user.role == "user":
        if item.user_id != current_user.id:
            raise HTTPException(status_code=403, detail="只能修改自己的库存")
    elif current_user.role == "business_manager":
        if item.user_id != current_user.id:
            sub = db.query(User).filter(User.id == item.user_id, User.manager_id == current_user.id).first()
            if not sub:
                raise HTTPException(status_code=403, detail="该库存属于非下属客户，无法修改")

    if inv_data.knowledge_base_id is not None: item.knowledge_base_id = inv_data.knowledge_base_id
    if inv_data.product_name is not None: item.product_name = inv_data.product_name
    if inv_data.specification is not None: item.specification = inv_data.specification
    if inv_data.quantity is not None: item.quantity = inv_data.quantity
    if inv_data.unit is not None: item.unit = inv_data.unit
    if inv_data.price is not None: item.price = inv_data.price
    if inv_data.cost_price is not None: item.cost_price = inv_data.cost_price
    if inv_data.notes is not None: item.notes = inv_data.notes
...
async def delete_inventory(
    ...
    if current_user.role == "user":
        if item.user_id != current_user.id:
            raise HTTPException(status_code=403, detail="只能删除自己的库存")
    elif current_user.role == "business_manager":
        if item.user_id != current_user.id:
            sub = db.query(User).filter(User.id == item.user_id, User.manager_id == current_user.id).first()
            if not sub:
                raise HTTPException(status_code=403, detail="该库存属于非下属客户，无法删除")

    db.delete(item)
    db.commit()
    return {"message": "库存删除成功"}


# ── Inventory AI Analysis ──────────────────────────

@app.post("/api/inventory/ai-analysis")
async def inventory_ai_analysis(
    body: dict,
    current_user: User = Depends(require_role(*READ_ROLES)),
    db: Session = Depends(get_db),
):
    """AI 库存统计分析 - 根据库存数据生成智能分析报告"""
    user_id = body.get("user_id")  # 可选：指定分析某个用户的库存

    # 获取库存数据（范围同 list_inventory 的权限逻辑）
    if current_user.role == "user":
        items = db.query(Inventory).filter(Inventory.user_id == current_user.id).all()
    elif current_user.role == "business_manager":
        if user_id and user_id != current_user.id:
            sub = db.query(User).filter(User.id == user_id, User.manager_id == current_user.id).first()
            if not sub:
                raise HTTPException(status_code=403, detail="无权查看该用户库存")
            items = db.query(Inventory).filter(Inventory.user_id == user_id).all()
        else:
            subordinate_ids = [u.id for u in db.query(User).filter(User.manager_id == current_user.id).all()]
            subordinate_ids.append(current_user.id)
    ...
    if not items:
        return {"analysis": "当前无库存数据，暂无法生成分析报告。请先添加库存后再进行分析。", "summary": {"total_products": 0, "total_value": 0, "total_quantity": 0}}
    ...
    product_summary = {}
    for i in items:
        inv_data_text += f"- 产品:{i.product_name} 规格:{i.specification or '无'} 数量:{i.quantity}{i.unit or '个'} 售价:{i.price} 成本:{i.cost_price or 0} 备注:{i.notes or '无'}\n"
        value = i.quantity * i.price
        cost = i.quantity * (i.cost_price or 0)
        total_value += value
        total_cost += cost
        total_quantity += i.quantity
        if i.product_name not in product_summary:
            product_summary[i.product_name] = {"count": 0, "quantity": 0, "value": 0, "cost": 0}
        product_summary[i.product_name]["count"] += 1
        product_summary[i.product_name]["quantity"] += i.quantity
        product_summary[i.product_name]["value"] += value
        product_summary[i.product_name]["cost"] += cost

    # 获取关联的知识库名称
    kb_ids = set(i.knowledge_base_id for i in items if i.knowledge_base_id)
    kb_names = {k.id: k.name for k in db.query(KnowledgeBase).filter(KnowledgeBase.id.in_(kb_ids)).all()} if kb_ids else {}

    # 按知识库分组
    kb_groups = {}
    for i in items:
        kb_name = kb_names.get(i.knowledge_base_id, "未关联知识库") if i.knowledge_base_id else "未关联知识库"
        if kb_name not in kb_groups:
            kb_groups[kb_name] = []
        kb_groups[kb_name].append(f"{i.product_name}({i.quantity}{i.unit or '个'})")

    # 构建分析请求
    summary_text = f"总产品种类:{len(product_summary)} 总库存数量:{total_quantity} 总库存价值:{total_value}元 总成本:{total_cost}元 毛利:{total_value-total_cost}元\n"
    summary_text += "\n各产品详情:\n"
    for pname, pdata in product_summary.items():
        margin = pdata["value"] - pdata["cost"]
        margin_rate = (margin / pdata["value"] * 100) if pdata["value"] > 0 else 0
        summary_text += f"  {pname}: 数量{pdata['quantity']} 销售额{pdata['value']}元 成本{pdata['cost']}元 毛利{margin}元({margin_rate:.1f}%)\n"
    if kb_groups:
        summary_text += "\n按知识库分组:\n"
        for kb_name, products in kb_groups.items():
            summary_text += f"  {kb_name}: {', '.join(products)}\n"

    # 调用 AI 生成分析
    analysis_prompt = f"""你是一位专业的健康产品库存管理分析师。请根据以下库存数据进行深度分析，给出专业建议。
...
1. 库存结构分析：各类产品的占比和分布
2. 成本效益分析：毛利率、利润贡献度
3. 库存风险预警：是否存在库存积压或短缺风险
4. 优化建议：库存调整、产品组合优化、成本控制建议
5. 趋势预测：基于当前数据给出未来采购和销售建议

请用◆板块◆和【子项】格式输出，简洁专业。"""
    ...
    try:
        ...
        if not AGNES_API_KEY:
            return {"analysis": "AI 分析服务未配置（缺少 API Key），请联系管理员。", "summary": {"total_products": len(product_summary), "total_value": total_value, "total_quantity": total_quantity, "total_cost": total_cost, "profit": total_value - total_cost}}
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(f"{AGNES_API_URL}/chat/completions", headers=headers, json=payload)
            resp.raise_for_status()
            data = resp.json()
            analysis_text = data["choices"][0]["message"]["content"]
    except Exception as e:
        analysis_text = f"AI 分析暂时不可用：{str(e)}。以上是原始数据摘要，建议稍后重试。"
    ...
    return {
        ...
        "summary": {
            ...
            "profit_rate": ((total_value - total_cost) / total_value * 100) if total_value > 0 else 0,
            "product_details": [{"name": n, "quantity": d["quantity"], "value": d["value"], "cost": d["cost"], "margin": d["value"] - d["cost"]} for n, d in product_summary.items()],
        }
    }

# ── Region CRUD API ──────────────────────────
LEVEL_LABELS = {"headquarters": "总部", "province": "省区", "district": "地区", "county": "县区", "user_area": "业务区域"}

LEVEL_ORDER = ["headquarters", "province", "district", "county", "user_area"]

def get_region_path(region_id, db: Session) -> str:
    """获取某个区域的完整路径（如：河南-郑州-金水区）"""
    ...
    while current_id and current_id not in visited:
        ...
        region = db.query(Region).filter(Region.id == current_id).first()
        if not region:
            break
...
def get_subordinate_region_ids(region_id, db: Session) -> list:
    """获取某个区域的所有下属区域ID（包括自身）"""
    if region_id is None:
        return []
    from models import Region
    ...
    while queue:
        ...
        if current in children_map:
            result.extend(children_map[current])
            queue.extend(children_map[current])
    return result


@app.get("/api/regions/tree")
async def get_region_tree(kb_id: int = None, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """返回层级树结构供 el-cascader 和 el-tree 使用，kb_id 可选过滤"""
    from models import Region
    query = db.query(Region)
    if kb_id:
        query = query.filter(Region.knowledge_base_id == kb_id)
    all_regions = query.order_by(Region.sort_order, Region.id).all()
    # 构建 {id: {value, label, name, level, parent_id, sort_order, children[]}} 树
    nodes = {}
    for r in all_regions:
        nodes[r.id] = {"value": r.id, "label": r.name, "name": r.name, "level": r.level, "parent_id": r.parent_id, "sort_order": r.sort_order, "knowledge_base_id": r.knowledge_base_id, "children": []}
    roots = []
    for r in all_regions:
    ...
    # Build level labels from unique levels in this tree
    levels_in_tree = set()
    for node in roots:
        _collect_levels(node, levels_in_tree)
    
    # Map level codes to Chinese labels
    level_map = {
        "province": "省级",
        "city": "市级", 
        "county": "县级",
    ...
    level_order = ["province", "city", "county", "district", "town", "village"]
    level_labels = {k: level_map.get(k, k) for k in levels_in_tree}
    
    return {"data": roots, "level_labels": level_labels, "level_order": [l for l in level_order if l in levels_in_tree]}


def _collect_levels(node, result_set):
    """Recursively collect all level values from a tree"""
    if node.get("level"):
        result_set.add(node["level"])
    for child in node.get("children", []):
        _collect_levels(child, result_set)


@app.get("/api/regions")
async def list_regions(kb_id: int = None, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """返回扁平列表，kb_id 可选过滤"""
    from models import Region
    query = db.query(Region)
    if kb_id:
        query = query.filter(Region.knowledge_base_id == kb_id)
    regions = query.order_by(Region.sort_order, Region.id).all()
    return {"data": [{"id": r.id, "name": r.name, "parent_id": r.parent_id, "level": r.level, "knowledge_base_id": r.knowledge_base_id} for r in regions]}

@app.post("/api/regions")
async def create_region(body: dict, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.role not in ("super_admin", "admin"):
        raise HTTPException(403, "无权限")
    from models import Region
    region = Region(name=body.get("name","").strip(), parent_id=body.get("parent_id"), level=body.get("level","district"), knowledge_base_id=body.get("knowledge_base_id"))
    db.add(region)
    db.commit()
    db.refresh(region)
...
async def update_region(region_id: int, body: dict, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.role not in ("super_admin", "admin"):
        raise HTTPException(403, "无权限")
    from models import Region
    ...
    if "parent_id" in body:
        region.parent_id = body["parent_id"]
    if "level" in body:
        region.level = body["level"]
    if "knowledge_base_id" in body:
        region.knowledge_base_id = body["knowledge_base_id"]
    db.commit()
    return {"message": "更新成功"}

@app.delete("/api/regions/{region_id}")
async def delete_region(region_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if current_user.role not in ("super_admin", "admin"):
        raise HTTPException(403, "无权限")
    from models import Region
    region = db.query(Region).filter(Region.id == region_id).first()
    if not region:
        raise HTTPException(404, "区域不存在")
    db.query(Region).filter(Region.parent_id == region_id).delete()
    db.delete(region)
    db.commit()
    return {"message": "删除成功"}
